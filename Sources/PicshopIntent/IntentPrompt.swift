...
public enum IntentPrompt {
    ...
    public static func systemInstructions(mode: EditorMode) -> String {
        ...
        - VIDEO MAGIC: autoCaptions (subtitles from the speech; text = style: classic|karaoke|reveal|boxed|minimal, also to restyle existing captions), removeCaptions, removeSilences (jump cuts: remove pauses in speech; amount 0.2 gentle … 0.45 tight), removeFillers (cut the "euh"/"um" hesitations and stutters), autoDuck (music dips under the voice, back up between sentences; amount = depth 0.3…0.9, 0 = off), animateText (how the latest title comes on: text = pop|rise|wipe|focus|drift, or "none"), speedRamp (ease into slow motion around the playhead or seconds, then back; amount = slowest speed, default 0.3), highlights (a recap made of the best moments; amount = length in seconds, default 30), splitScenes (split the clips at every shot change; scope "all" or the selected clip), trackSubject (a text/sticker/picture overlay follows the moving subject under it; target "text"|"image"|"video"|"shape"; amount 0 = stop following), cutWords (edit by text: text = the exact words to cut where they are spoken; scope "all" = every time; target "sentence" = the whole sentence around them), syncToBeat (move every cut onto the music's beat), smartReframe (aspect + follow the subject: "passe en vertical en suivant la personne"), kenBurns (slow camera move; scope "all"; amount 0 removes it), enhanceVoice (remove background noise from speech; scope "all"), matchColor (give every clip the colours of clipNumber)
