...
final class RuleParserTests: XCTestCase {
    ...
    func testVideoTransitionsMusicAndFrames() {
        ...
        XCTAssertEqual(music.scope, .current, "a new sound is another track, not a replacement")
    ...
    /// Several sound tracks, each addressable by number, placed at a time, faded,
    /// moved, muted or removed — the words a Final Cut / Vegas user reaches for.
    func testSoundTracks() {
        let context = IntentContext(mode: .video, clipCount: 1, playheadSeconds: 5, timelineDuration: 30)
        let second = first("ajoute un deuxième son à 10 secondes", context: context)
        XCTAssertEqual(second.action, .addMusic)
        XCTAssertEqual(second.time, 10)
        XCTAssertEqual(second.scope, .current)
        let here = first("add a voice over here", context: context)
        XCTAssertEqual(here.action, .addMusic)
        XCTAssertEqual(here.time, 5, "\"here\" is the playhead")
        let replace = first("remplace la musique par un morceau jazz", context: context)
        XCTAssertEqual(replace.action, .addMusic)
        XCTAssertEqual(replace.scope, .selection, "replacing swaps the existing music")
        let removeSecond = first("supprime la deuxième piste", context: context)
        XCTAssertEqual(removeSecond.action, .removeMusic)
        XCTAssertEqual(removeSecond.clipIndex, 2)
        let lower = first("baisse la deuxième piste", context: context)
        XCTAssertEqual(lower.action, .setVolume)
        XCTAssertEqual(lower.scope, .selection)
        XCTAssertEqual(lower.clipIndex, 2)
        XCTAssertEqual(lower.amount, .relative(-0.25))
        let percent = first("mets la musique à 30 pour cent", context: context)
        XCTAssertEqual(percent.action, .setVolume)
        XCTAssertEqual(percent.amount, .absolute(0.3))
        let mute = first("coupe la musique", context: context)
        XCTAssertEqual(mute.action, .mute)
        XCTAssertEqual(mute.scope, .selection, "muting the music is not removing it")
        let fade = first("fais un fondu de sortie de 2 secondes sur la musique", context: context)
        XCTAssertEqual(fade.action, .fadeAudio)
        XCTAssertEqual(fade.amount, .absolute(2))
        XCTAssertEqual(fade.text, "out")
        let move = first("move the last track to 12 seconds", context: context)
        XCTAssertEqual(move.action, .moveAudio)
        XCTAssertEqual(move.clipIndex, -1)
        XCTAssertEqual(move.time, 12)
